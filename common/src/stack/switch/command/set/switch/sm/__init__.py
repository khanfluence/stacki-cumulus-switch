...
class Command(stack.commands.SwitchArgumentProcessor,
	...
	def run(self, params, args):
		...
		opts = shlex.split(next(sw['options'] for sw in ib_sw_nets if sw['host'] == sm_switch))
		for opt in opts:
		...
		if not fabric:
			raise CommandError(self.owner, 'switch {sm_switch} does not have its ibfabric set')

		for ib_sw in ib_sw_nets:
			if f'ibfabric={fabric}' not in shlex.split(ib_sw['options']):
				continue
